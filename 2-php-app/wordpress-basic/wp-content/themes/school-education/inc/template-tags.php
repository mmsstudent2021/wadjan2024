	...
	function school_education_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( true === $show_meta_categories ) {
				...
				$categories_list = get_the_category_list( esc_html__( ', ', 'school-education' ) );
				if ( $categories_list && school_education_categorized_blog() ) {
			...
			if ( true === $show_meta_tags ) {
				/* Translators: used between list items, there is a space after the comma. */
				$tags_list = get_the_tag_list( '', esc_html__( ', ', 'school-education' ) );
				if ( $tags_list ) {
